...
class Cart extends CI_Controller
{
    ...
    public function new_purchase() 
    {
        ...
        $this->db->select('*')->where('status', 'Selling')->where('category !=', '1');
    ...
    public function walletPaymentProcess($o_id) {
        ...
        /** Calculation for percentagewise amount from coupons and product wallet **/
        
        if($wallet_prod > $percent_20){
            $wallet_prod_amt = $percent_20;
        }
        else{
            $wallet_prod_amt = $wallet_prod;
        }
        if($coupon > $percent_20){
            $coupon_amt = $percent_20;
        }
        else{
            $coupon_amt = $coupon;
        }
        ...
        if($coupon != NULL and $prod_wall != NULL){
            $coupn_wallet = $coupon_amt + $wallet_prod_amt;
            $cart_total   = $this->cart->total() - $coupn_wallet;
        }
        
        elseif($coupon != NULL){
            $cart_total  = $this->cart->total() - $coupon_amt;
        }
            ...
            redirect(site_url('cart/pre_checkout'));
                ...
                if ($cart = $this->cart->contents()){
                    foreach ($cart as $item):
                        ...
                        $this->earning->reg_earning($this->session->user_id, $this->db_model->select('sponsor', 'member', array('id' => $this->session->user_id)), $item['id']);
                    endforeach;
                } 
