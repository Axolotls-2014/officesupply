...
class Deduction_model extends CI_model {
  ...
  var $order = array('deduction_id' => 'desc'); // default order 
  ...
  public function add_record($data)
  {
    ...
    if($this->db->insert('hr_deductions',$data)){
  ...
  public function edit_record($data,$id)
  {
    ...
    if($this->db->update('hr_deductions',$data)){
      return $id;
    }
    else{
      return FALSE;
  ...
  // public function delete_record($deduction_id,$soft_delete = TRUE)
  // {
  ...
  public function get_single_record($deduction_id)
  {
    $this->db->select('*');
    $this->db->from('hr_deductions');
    $this->db->where('deduction_id',$deduction_id);
  ...
  private function _get_datatables_query()
  {
    ...
    if(isset($_POST['from_date']) && !empty($_POST['from_date'])) 
    {
      ...
      $this->db->where('deduction_date >=', $from_date);
    }

    if(isset($_POST['to_date']) && !empty($_POST['to_date'])) 
    {
      $to_date = date('Y-m-d', strtotime($_POST['to_date']));
      $this->db->where('deduction_date <=', $to_date);
    }
    $i = 0;
  ...
  public function get_records_by_employee_id($employee_id)
  {
      return $this->db->select('
                      ...
                      ->join('hr_employees e','e.employee_id = d.employee_id','left')
