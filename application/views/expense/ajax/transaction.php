<table class="table table-striped">
  ...
  <tr>
    ...
    <td><?=($value->to_account == OTHER_SUPPLIER) ? $this->ledger_model->get_single_record($value->to_account)->title : $value->to_ledger_title?></td>
    <td><?=$value->amount?></td>
    <td><?=date('d-M-Y H:i:s', strtotime($value->created_date))?></td>
    <td>
      <?php 
        if($value->type != EXPENSE_TRANSACTION_TYPE)
        {
      ?>
