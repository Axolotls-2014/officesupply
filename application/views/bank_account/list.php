              ...
              <div class="card-tools">
                ...
                <ul class="nav nav-pills ml-auto">
                  ...
                  <li class="nav-item  ml-2">
                    <a class="nav-link import_bank_account_modal btn-sm btn-secondary text-white" href="#" data-tt="tooltip" title="Click here to Import Bank account in Bulk using CSV file">
                      <i class="fas fa-file-import"></i> Import Bank Account
                    </a>
                  </li>
                  <?php 
                    }
              ...
              <table id="example" class="table table-bordered table-striped">
                <thead>
                  <tr>
                    ...
                    <th><?=$this->lang->line('bank_account_status')?></th>
...
<div class="example-modal">
  <div class="modal fade" id="import_bank_account_modal" data-backdrop="static" data-keyboard="false">
...
function initialize_datatable()
    {
      ...
      "ajax": {
          "url": "<?php echo site_url('bank_account/ajax_list')?>",
          "type": "POST",
          "data":  {
            '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'
          }
  ...
  $(document).ready(function(e){
    ...
    /*************************** End Dynamic Product List with Datatables ****************************/
      ...
      $(document).on('click','.change_status',function (e) {
        ...
        if($(this).find('span').hasClass('badge-success'))
          message = 'Are you sure want to deactivate the bank account.';
        else
          message = 'Are you sure want to activate the bank account.';
      ...
      /* Import bank account using CSV function Begin */

    $(document).on('click', ".import_bank_account_modal" ,function(event){
      event.preventDefault();

      $.ajax({
        url: "<?php echo base_url('bank_account/import_bank_account')?>",
        type: "GET",
        dataType: "JSON",
        success: function(data){
          $('#import_bank_account_modal').find('.modal-content').html(data.import_bank_account_modal_body);
          $('#import_bank_account_modal').modal('show');
        },
    ...
    $(document).on('submit','form#importbankAccountForm',function(event){
      // event.preventDefault();

      $('form#importbankAccountForm #importTaxSubmit').text('<?=$this->lang->line("please_wait")?>').attr('disabled','disabled');
      var formData = $('form#importbankAccountForm').serialize();

      var isError = false;

      $('form#importbankAccountForm .field_validation').each(function() {
          ...
          if(value==null || value==""){
            $("form#importbankAccountForm  #err_"+id).text(field+ " field is required.");
            $('form#importbankAccountForm  #'+id).addClass('is-invalid');
            isError = true;
          }
          else
          {
            $("form#importbankAccountForm #err_"+id).text("");
            $('form#importbankAccountForm #'+id).removeClass('is-invalid');
            $('form#importbankAccountForm #'+id).addClass('is-valid');
          }
      ...
      if(isError == true)
      {
        $('form#importbankAccountForm #importbankAccountSubmit').text('<?=$this->lang->line("submit")?>').removeAttr('disabled');
        return false;
      }
      else
      {
    ...
    /* Import bank account using CSV function End */
  });
</script>