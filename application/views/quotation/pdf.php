...
<div style="font-family: Arial, sans-serif; line-height: 1.6;">
    ...
    <h4 style="margin: 2px 0 0 0; text-align: center;">
        ...
        <!-- Quotation and Transport Details -->
       <table class="items-table">
            <tr class="heading">
                <td colspan="4">Bill To</td>
                <td colspan="4">Ship To</td>
                <td colspan="4">Quotation Details</td>
            </tr>
            <tr class="details">
                <td colspan="4">
                ...
                <td colspan="4">
                    ...
                    Due Date: <?= date('d-m-Y', strtotime($quotation->quotation_date . ' + ' . $quotation->due_days . ' days')); ?><br>
        ...
        <table>
            ...
            <!-- Freight Charges -->
            <?php if($quotation->additional_cost_type == 'external' && $quotation->additional_cost_amount > 0): ?>
            <tr class="item">
                <td><?= $i++; ?></td>
                ...
                <td>N/A</td>
                <td>1</td>
                <td>₹ <?= number_format_i($quotation->freight_selling_price); ?></td>
                <td>₹ <?= number_format_i($quotation->freight_taxable_value); ?></td>
                <td>
                    ₹ <?= number_format_i($quotation->freight_tax_amount); ?> 
                    (<?= $quotation->freight_tax_rate; ?>%)
                </td>
                <td>₹ <?= number_format_i($quotation->freight_sub_total); ?></td>
            </tr>
            <?php 
                // Add freight values to totals
                $total_taxable_value += $quotation->freight_taxable_value;
                $total_tax += $quotation->freight_tax_amount;
                $total_subtotal += $quotation->freight_sub_total;
            endif; ?>
        </table>
            ...
            <tr class="details">
                ...
                <td colspan="3" style="line-height: 1.6; vertical-align: top;">
                    ...
                    <div style="display: flex; justify-content: space-between; font-weight: bold; color: #000;">
                        <strong>Total:</strong> 
                        <span style="text-align: right;">₹ <?= number_format_i(round($quotation->total)); ?></span>
                    </div>
                </td>
            </tr>
        </table>
        ...
        <!-- QR Code -->
        <?php if(!empty($quotation->qr)): ?>
        <div style="text-align: right; margin-top: 10px;">
            <?php 
            $qr_path = FCPATH . 'uploads/qr_codes/' . $quotation->qr; 
            if (file_exists($qr_path)): 
            ?>
                <span>Scan to view quotation:</span>
                <img src="<?= base_url('uploads/qr_codes/' . $quotation->qr); ?>" style="width: 60px; height: auto;">
            <?php endif; ?>
        </div>
        <?php endif; ?>
