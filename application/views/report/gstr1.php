  	...
  	<div class="wrapper">
	  	<div class="content-wrapper">
		      ...
		      <div class="row mb-2">
		        <div class="col-sm-12">
		          <ol class="breadcrumb breadcrumb-custom float-sm-left">
		            ...
		            <li class="breadcrumb-item active"><?=$this->lang->line('gstr1_report')?></li>
		          </ol>
		        </div>
		      </div>
		    </section>
            ...
            <!-- general form elements disabled -->
            	<form role="form" id="gstr1ReportForm" name="gstr1ReportForm" method="POST" action="<?php echo base_url("report/gstr1") ?>" target="_blank">
	            	<div class="card card-primary">
	                  ...
	                  <div class="row">
	                    ...
	                    <div class="col-sm-3">
	                    	<div class="form-group">
                          <label for="customer"><?=$this->lang->line('gstr1_invoice_type')?></label>
                          <select class="form-control form-control-sm select2bs4" name="invoice_type" id="invoice_type" width="100%" class="add-row">
                            <option value="<?=B2B?>"><?=strtoupper(B2B)?></option>
                            <option value="<?=B2CL?>"><?=strtoupper(B2CL) ?></option>
                            <option value="<?=B2CS?>"><?=strtoupper(B2CS)?></option>
                            <option value="<?=CDNR?>"><?=strtoupper(CDNR)?></option>
                            <option value="<?=CDNUR?>"><?=strtoupper(CDNUR)?></option>
                          </select>
                          <span id="err_invoice_type" class="error invalid-feedback"><?=form_error('invoice_type');?></span>
                        </div>
	                    </div>
	                    <div class="col-sm-3">
	                    	<div class="form-group">
		              ...
		              <div class="card-footer">
		              	...
		              	<button type="submit" name="submit" id="submitGstr1Report" class="btn btn-info"><?=$this->lang->line('search')?></button>
		              </div>
	            	</div>
	            </form>
	            <div class="card card-primary">
	              <div class="card-header">
	                <h3 class="card-title"><?=$this->lang->line('gstr1_report')?></h3>
	              ...
	              <div class="card-body gstr1_list">
	                Click on "Search" button to generate report.
	              </div>
	            </div>
	            <!-- /.card -->
	          </div>
...
<script type="text/javascript">
	$(document).ready(function(e){
		$('#submitGstr1Report').click(function(e){
			// e.preventDefault();
			...
			else
			{
				...
				var formData = $('#gstr1ReportForm').serialize();
				$(this).text('<?=$this->lang->line('searching')?>');

				setTimeout(function(){ 

					$.ajax({
		        url: '<?php echo base_url("report/gstr1") ?>',
		        type: 'POST',
		        dataType : 'json',
		        data: formData,                       
		        success: function (response) {
		          $('.gstr1_list').html(response.gstr1);
		          $('#submitGstr1Report').text('<?=$this->lang->line('search')?>');
		        },
		        error: function () 
		        { 
		        	$('#submitGstr1Report').text('<?=$this->lang->line('search')?>');
		...
		$('#export').click(function(e){
			...
			$('#submitGstr1Report').trigger('click');  
			$('#action_type').val('');
		});
