...
<li class="breadcrumb-item"><a href="<?php echo base_url('auth');?>"><?=$this->lang->line('home')?></a></li>
                ...
                <div class="col-md-12">
                                    ...
                                    <div class="col-sm-3">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control form-control-sm select2bs4" name="pid" id="pid" width="100%">
                                                ...
                                                ?>
                                                    <!--<option value="<?=$value->id;?>"  <?= ($value->pid == $sel_pid) ? 'selected' : '' ?>  >-->
                                                    <!--    <?= $value->name;?>-->
                                                    <!--</option>-->
                                                    <option value="<?=$value->id;?>"  <?= ($value->id == $sel_pid) ? 'selected' : '' ?>  >
                                                        <?= $value->name;?>
                                                    </option>
                                                <?php 
                                                        }
                    ...
                    <div class="card card-primary">
                        <div class="card-header">
                            ...
                            <h3 class="card-title">Stock Movement Summary</h3>
    ...
    ?>
       <tr data-pid="<?= $row->product_id ?>">
            ...
            <td><?= $row->product_alert_limit ?></td>
            <td class="text-success"><?= number_format($row->in_qty, 2) ?></td>
            <td><?= number_format($row->pur_amt, 2) ?></td>
            <td class="text-danger"><?= number_format($row->out_qty, 2) ?></td>
            <td><?= number_format($row->sale_amt, 2) ?></td>
            <td><b><?= number_format($row->closing_stock, 2) ?></b></td>
            <td><b><?= number_format($row->closing_value, 2) ?></b></td>
        </tr>
    <?php 
            }
        } else {
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <!--<td><?=number_format($total_closing_cost, 2)?></td>-->
                                         <td><b><?= number_format($total_opening_quantity, 2) ?></b></td>
                                        <td><b>₹ <?= number_format($total_opening_cost, 2) ?></b></td>
                                        <td><b class="text-primary"><?= number_format($total_closing_quantity, 2) ?></b></td>
                                        <td><b class="text-success">₹ <?= number_format($total_closing_cost, 2) ?></b></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
...
$(document).ready(function () {
   ...
   $('#pid').on('change', function () {
        ...
        if (selectedId == "") {
            ...
            $('#stock-value-body tr').show();
        } else {
            // Hide all rows first
            $('#stock-value-body tr').hide();
            // Show only the row that matches the ID from our updated dropdown
            $('#stock-value-body tr[data-pid="' + selectedId + '"]').show();
        }
    });
});
</script>