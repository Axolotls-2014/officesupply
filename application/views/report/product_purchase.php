...
<li class="breadcrumb-item"><a href="<?php echo base_url('auth');?>"><?=$this->lang->line('home')?></a></li>
		            ...
		            <li class="breadcrumb-item active"><?=$this->lang->line('product_purchase_report')?></li>
		          </ol>
		        </div>
		      </div>
		    </section>
            ...
            <!-- general form elements disabled -->
            	<form role="form" id="productReport" name="productReport" method="POST" action="<?php echo base_url("report/product_purchase") ?>" target="_blank">
	            ...
	            <div class="card card-primary">
	              <div class="card-header">
	                <h3 class="card-title"><?=$this->lang->line('product_purchase_report')?></h3>
	                <div class="card-tools">
...
<script type="text/javascript">
	$(document).ready(function(e){
      ...
      if(action_type == 'pdf')
			...
			else
			{
				...
				setTimeout(function(){ 
					...
					$.ajax({
		        url: '<?php echo base_url("report/product_purchase") ?>',
		        type: 'POST',
		        dataType : 'json',
		        data: formData,                       
      ...
      var headers = [];
      $("#product-purchase-data thead th").each(function () {
          headers.push('"' + $(this).text() + '"');
      });
      data.push(headers);

      // Get the table rows with the "customer_row" class, trim the data
      $("#product-purchase-data tbody tr").each(function () {
          var rowData = [];
      ...
      var footerData = [];
      $("#product-purchase-data tfoot th").each(function () {
          var cellData = $(this).text().trim(); // Trim the data
      ...
      link.setAttribute("download", "ProductPurchase.csv");
