  ...
  <thead>
	...
	<tbody>
      ...
      {	
      	...
      	$paid_amount = $this->transaction_model->get_total_transaction_amount($value->id, PURCHASE_MODULE, PAYMENT_TRANSACTION_TYPE)+ $this->transaction_model->get_total_transaction_amount($value->id,PURCHASE_MODULE,CREDIT_TRANSACTION_TYPE);
        ...
        if($dueDate < $today) 
				{
				    $no_of_days *= -1;
				    $no_of_days = '(<span class="text-danger">'.$no_of_days.' days</span>)';
				}
				else
        {
		...
		<tr>
			<td><?=$i++?></td>
      <td>
        <a href="<?=base_url('purchase/view/'.base64_encode($value->id))?>" target="_blank">
          <?=$value->reference_no?>  
        </a>
      </td>
