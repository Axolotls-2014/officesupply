  ...
  <div class="wrapper">
    <div class="content-wrapper">
          ...
          <div class="row mb-2">
            <div class="col-sm-12">
              <ol class="breadcrumb breadcrumb-custom float-sm-left">
                ...
                <li class="breadcrumb-item "><a href="<?=base_url('purchase_order')?>"><?=$this->lang->line('purchase_order_header')?></a></li>
                <li class="breadcrumb-item active"><?=$this->lang->line('purchase_order_edit')?></li>
              </ol>
            </div>
          </div>
      </section>
      ...
      <section class="content">
        <div class="row">
          <div class="col-md-12">
            <form class="form-horizontal" name="editPurchaseOrderForm" id="editPurchaseOrderForm" method="POST" action="<?=base_url('purchase_order/edit')?>">
              <div class="card">
                <div class="card-header"><?=$this->lang->line('purchase_order_edit')?></h6>
                  <div class="card-tools">
                    <ul class="nav nav-pills ml-auto">
                      ...
                      <li class="nav-item  ml-2">
                        <a class="nav-link reset btn-sm btn-warning" href="#" data-tt="tooltip" title="Click here to Reset Purchase Order Items">
                      ...
                      <li class="nav-item ml-2">
                        <a class="nav-link active" href="<?=base_url('purchase_order')?>" data-tt="tooltip" title="Click here to show purchase order list"><i class="fas fa-arrow-left mr-2"></i>Back</a>
                      </li>
                    ...
                    <div class="row">
                      ...
                      <div class="col-sm-3">
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control datepicker" id="purchase_order_date" name="purchase_order_date" value="<?=date('d-m-Y', strtotime($purchase_order->purchase_order_date))?>" placeholder="Invoice Date">
                          ...
                          <div class="input-group input-group-sm">
                            ...
                            ?>
                              <option value="<?=$value->id;?>"
                                <?php 
                                  if($value->id == $purchase_order->warehouse_id)
                                    echo ' selected';
                                ?>
                              >
                                <?= $value->name;?>
    ...
    <div class="form-group">
        ...
        <span id="err_warehouse_id" class="error invalid-feedback"><?=form_error('warehouse_id');?></span>
                      ...
                      <div class="col-sm-3">
                        <div class="form-group">
                          <label for="supplier"><?=$this->lang->line('purchase_order_supplier')?><span class="validation-color">*</span>
                          </label>
                          <div class="input-group input-group-sm">
                            <select class="form-control form-control-sm select2bs4 field_validation" name="supplier_id" id="supplier_id" width="100%" class="add-row" placeholder="<?=$this->lang->line('purchase_order_supplier')?>">
                              ...
                              ?>
                                <option value="<?=$value->id;?>"
                                  <?php 
                                    if($value->id == $purchase_order->supplier_id)
                                      echo ' selected';
                                  ?>
                                >
                                  <?= $value->company_name;?>
...
<div class="row">
  ...
  <div class="col-sm-4">
    <div class="form-group">
      ...
      <select name="due_days" id="due_days" class="form-control">
        <option value="">Select</option>
        <?php if(!empty($due_days_options)): ?>
          <?php foreach ($due_days_options as $row): ?>
           <option value="<?= $row->due_day ?>"
    data-terms="<?= htmlspecialchars($row->terms_and_condition) ?>"
    <?= ($purchase_order->due_days == $row->due_day) ? 'selected' : '' ?>>
    <?= $row->due_day ?> Days
</option>
          <?php endforeach; ?>
        <?php endif; ?>
  ...
  <div id="due_days_terms" class="form-control" style="background:#f8f9fa;">
    <?= $purchase_order->payment_terms ?? '' ?>
  </div>
</div>

<input type="hidden" name="payment_terms" id="payment_terms"
       value="<?= $purchase_order->payment_terms ?? '' ?>">
  </div>
  ...
  <div id="due_date_display" class="form-control" style="background:#f8f9fa;">
    <?= !empty($purchase_order->due_date) 
...
<input type="hidden" name="due_date" id="due_date"
       value="<?= !empty($purchase_order->due_date) 
           ? date('d-m-Y', strtotime($purchase_order->due_date)) 
           : '' ?>">
  </div>
                      ...
                      <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <div class="input-group input-group-sm">
                                ...
                                <input type="hidden" value="<?= $purchase_order->document ?>" name="document[]" id="document">
                            </div>
                            <span id="fileTypeError" class="error-message"></span>
                            <?php
                              $document_filenames = explode(',', $purchase_order->document);
                    ...
                    <div class="row">
                      <div class="col-sm-12">
                        <div class="form-group">
                          <label><?=$this->lang->line('purchase_order_items')?></label>
 ...
 <table class="table items table-striped table-bordered table-condensed table-hover product_table" 
    ...
    <tbody id="product_table_body">
      <?php $sr_no = 1; foreach ($purchase_order_items as $row): ?>
       ...
       <td>
    <span id="price_span">
        <input type="number" class="form-control text-right" name="cost" step="0.01" value="<?=($row->price > 0 ? $row->price : $row->cost)?>">
        <input type="hidden" class="form-control text-right" name="hidden_cost" step="0.01" value="<?=($row->price > 0 ? $row->price : $row->cost)?>">
    </span>
</td>
          <td>
      ...
      <?php if($purchase_order->additional_cost_type): ?>
    <tr class="product_row freight-row">
        <td>
            <span class="delete_item"><i class="fas fa-minus-circle text-danger"></i></span>
            <input type="hidden" name="freight_id" value="freight">
            <input type="hidden" name="freight_type" value="<?=$purchase_order->additional_cost_type?>">
            <input type="hidden" name="freight_tax_rate" value="0">
        </td>
        <td><?=$sr_no++?></td>
        <td><span name="freight_name">Freight Charges (<?=$purchase_order->additional_cost_type?>)</span></td>
        ...
        <td>
            <input type="number" class="form-control freight-amount" name="freight_cost" 
                   value="<?=$purchase_order->freight_amount?>" step="0.01" min="0">
        </td>
        <td>—</td>
        <td><span name="freight_taxable_value"><?=$purchase_order->freight_taxable_value?></span></td>
        <td class="tax_td">N/A</td>
        <td><span name="freight_sub_total"><?=$purchase_order->freight_sub_total?></span></td>
    </tr>
<?php endif; ?>
    </tbody>
  </table>
                          ...
                          <div class="controls">
                            <div class="tabbable">
                              <ul class="nav nav-tabs" id="custom-content-below-tab" role="tablist">
                                ...
                                <li class="nav-item">
                                  <a class="nav-link" href="#bank_detail" data-toggle="pill"><?php echo $this->lang->line('purchase_order_bank_detail'); ?></a>
                                </li>
                                <li class="nav-item">
                              ...
                              <div class="tab-content">
                                <div class="tab-pane active" id="external_note">
                                  <textarea class="col-sm-12 form-control" name="external_note" rows="4" placeholder="<?php echo $this->lang->line('purchase_order_external_note'); ?>"><?=str_replace("<br />","",$purchase_order->external_note)?></textarea>
                                </div>
                                <div class="tab-pane" id="internal_note">
                                  <textarea class="col-sm-12 form-control" name="internal_note" rows="4" placeholder="<?php echo $this->lang->line('purchase_order_internal_note'); ?>"><?=str_replace("<br />","",$purchase_order->internal_note)?></textarea>
                                </div>
                                <div class="tab-pane" id="bank_detail">
                                  <textarea class="col-sm-12 form-control" name="bank_detail" rows="4" placeholder="<?php echo $this->lang->line('purchase_order_bank_detail'); ?>"><?=str_replace("<br />","",$purchase_order->bank_detail)?></textarea>
                                </div>
                                <div class="tab-pane" id="terms_and_condition">
                                  <textarea class="col-sm-12 form-control" name="terms_and_condition" rows="4" placeholder="<?php echo $this->lang->line('purchase_order_terms_and_condition'); ?>"><?=str_replace("<br />","",$purchase_order->terms_and_condition)?></textarea>
                                </div>
                              </div>
                            </div>   
                          </div>
  ...
  <div class="modal fade" id="reset_model" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <p>
            <?php echo "Are you sure want to reset this purchase order ?";?>
          </p>
        </div>
        <div class="modal-footer">
...
<script type="text/javascript">
  ...
  $(document).ready(function(e){
          ...
          // On page load, ensure freight row values are correct for edit page
     if($('.freight-row').length > 0) {
        // Try to get amount from multiple possible sources
        var freightAmount = parseFloat($('.freight-row input[name="freight_cost"]').val()) || 0;
        
        // If still 0, check the hidden fields or spans
        if(freightAmount === 0) {
            freightAmount = parseFloat($('.freight-row span[name="freight_taxable_value"]').text()) || 0;
        }
        if(freightAmount === 0) {
            freightAmount = parseFloat($('#additional_cost_amount').val()) || 0;
        }
        
        if(freightAmount > 0) {
            $('.freight-row input[name="freight_cost"]').val(freightAmount);
            $('.freight-row span[name="freight_taxable_value"]').text(freightAmount.toFixed(2));
            $('.freight-row span[name="freight_sub_total"]').text(freightAmount.toFixed(2));
            $('#additional_cost_amount').val(freightAmount);
        }
        calculateGrandTotal();
    }
   ...
   $(document).on('click', ".add_product_modal" ,function(){
      ...
      /*add_product method remove $(document).on('click', ".add_product_modal" ,function(){
  ...
  $(document).on('submit','#addProductForm',function(e){
    ...
    $.ajax({
        ...
        success: function(data) {
            var product = data.product;
            var discounts = data.discount; // FIXED: Get discounts from response
            
            if(!is_product_exist_in_row(product.id)) {
                add_row(product, discounts); // Now discounts is defined
            } else {
                highlight_row(product.id);
            }
...
/* add row direct product not adding
function add_row(data)
{
  ...
  // FIXED: Initialize taxable_value as 0.00 instead of product.cost
  ...
  // Get current row count for Sr.No (excluding freight row to avoid counting it)
  var rowCount = $("#product_table_body tr:not(.freight-row)").length + 1;
  ...
  // Price/Unit column (visible) - FIXED: changed name from "price" to "cost"
  cols += '<td>'
  ...
  // Taxable Value column - FIXED: now uses 0.00 initial value
  ...
  calculateRow(newRow);
  calculateGrandTotal();  // ADD THIS LINE
  reindexSerialNumbers(); // ADD THIS LINE to fix Sr.No ordering
}*/
// Edit.php madhe ha add_row function replace kara
function add_row(product, discounts) {
    ...
    cols += '<td class="text-center sr-no">' + rowCount + '<\/td>';
    cols += '<td><span name="product_name">'+product.name+'</span><br/><small><span name="description">'+(product.description ? product.description : '')+'</span></small><\/td>';
    cols += '<td><textarea name="product_remark" rows="1" cols="20"><\/textarea><\/td>';
    cols += '<td><span name="hsn">'+(product.hsn ? product.hsn : 'NA')+'</span><\/td>';
    ...
    cols += '<td class="text-right">'
            ...
            + '<\/td>';
    cols += '<td>'+select_discount+'<\/td>';
    cols += '<td class="text-right"><span name="taxable_value">0.00</span><\/td>';
    cols += '<td class="tax_td">'+tax+'<\/td>';
    cols += '<td class="text-right"><span name="subtotal">0.00<\/span><\/td>';
...
*/
     ...
     /* $("table.product_table").on('change', 'input[name^="cost"], input[name^="quantity"], input[name^="selling_price"], select[name^="item_discount"]', function (event) {
...
$("table.product_table").on('change', 'input[name^="cost"], input[name^="price"], input[name^="quantity"], input[name^="selling_price"], select[name^="item_discount"]', function (event) {
    if($(this).attr('name') == 'item_discount') {
        var discount_id = $(this).val();
        var tr = $(this).closest('tr');
        ...
        if(discount_id != '' && discount_id != null) {
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    console.log("Discount AJAX Error:", error);
                }
            });
        } else {
            // No discount selected - reset values
...
/*  function calculateRow(row) {
  ...
  function calculateRow(row) {
    ...
    if(costInput.length === 0) {
        costInput = row.find('input[name^="price"]');
    }
    ...
    if(discount_type == 0) {
        ...
        final_discount_value = (base_amount * discount_value) / 100;
    }

    // Apply discount to get taxable value
    var taxable_value = base_amount - final_discount_value;
    
    // Get tax rates
    var igst_rate = 0, cgst_rate = 0, sgst_rate = 0;
    ...
    // Total price = Taxable value + Total Tax
    var total_tax = igst_tax + cgst_tax + sgst_tax;
    var subtotal = taxable_value + total_tax;

    // Update tax display based on transaction type
    if(company_country_id == supplier_country_id) {
        if(company_state_id == supplier_state_id) {
            // CGST/SGST case
            row.find('span[name^="i_tax"]').text('0.00');
            row.find('span[name^="igst"]').text('0');
            row.find('span[name^="c_tax"]').text(cgst_tax.toFixed(2));
            ...
            row.find('span[name^="cgst"]').text('0');
            row.find('span[name^="s_tax"]').text('0.00');
            row.find('span[name^="sgst"]').text('0');
        }
    } else {
        // International transaction - No tax
        row.find('span[name^="i_tax"]').text('0.00');
        row.find('span[name^="igst"]').text('0');
        row.find('span[name^="c_tax"]').text('0.00');
        row.find('span[name^="cgst"]').text('0');
...
/*total_taxable_value += freight_amount(04-05-2026)
...
function reindexSerialNumbers() {
    var normalRowCount = 0;
    $("#product_table_body tr").each(function() {
        if (!$(this).hasClass('freight-row')) {
            normalRowCount++;
            $(this).find('td:eq(1)').text(normalRowCount);
        }
    });
}
...
function reindexSerialNumbers() {
    ...
    $('form#editPurchaseOrderForm .field_validation').each(function() {
      var id = $(this).attr('id');
      var value = $(this).val();
      var field = $(this).attr('placeholder');

      if(value==null || value=="") {
        $("form#editPurchaseOrderForm #err_"+id).text(field+ " field is required.").fadeIn('slow');
        $('form#editPurchaseOrderForm #'+id).addClass('is-invalid');
        isError = true;
      } else {
        $("form#editPurchaseOrderForm #err_"+id).text("").fadeOut('slow');
        $('form#editPurchaseOrderForm #'+id).removeClass('is-invalid');
        $('form#editPurchaseOrderForm #'+id).addClass('is-valid');
      }
    });
    ...
    if ($('#freight_data').length === 0) {
      $('<input>').attr({
        ...
        id: 'freight_data',
        name: 'freight_data'
      }).appendTo('#editPurchaseOrderForm');
    }
 ...
 var productDataArray = [];
    ...
    if(isError == true) {
      ...
      $("form#editPurchaseOrderForm .field_validation").on("blur keyup change",  function (event){
        var id    = $(this).attr('id');
        var value = $(this).val();
        var field = $(this).attr('placeholder');
  ...
  let days = selected.val();
  let terms = selected.data('terms');

  if (!days) {
    $('#due_days_terms').html('');
    $('#payment_terms').val('');
    $('#due_date').val('');
    $('#due_date_display').html('');
    return;
  }

  // set payment terms
  $('#due_days_terms').html(terms);
  ...
  let parts = poDate.split('-'); // dd-mm-yyyy
  let baseDate = new Date(parts[2], parts[1] - 1, parts[0]);
  ...
  $('#due_date_display').html(formatted);
});
...
/*  $('#additional_cost_type').change(function () {
    ...
    // Update freight values when freight_cost changes
  $(document).on('input', 'input[name="freight_cost"]', function() {
      let amount = parseFloat($(this).val()) || 0;
      let row = $(this).closest('tr');
      
      // Update the taxable value and subtotal spans
      row.find('span[name="freight_taxable_value"]').text(amount.toFixed(2));
      row.find('span[name="freight_sub_total"]').text(amount.toFixed(2));
      
      // Update the additional cost amount field
      $('#additional_cost_amount').val(amount);
  ...
  $('#additional_cost_amount').on('input', function() {
      ...
      if(row.length > 0) {
          row.find('input[name="freight_cost"]').val(amount);
          row.find('span[name="freight_taxable_value"]').text(amount.toFixed(2));
          row.find('span[name="freight_sub_total"]').text(amount.toFixed(2));
          calculateGrandTotal();
      }
  ...
  // Additional cost type change handler - FIXED for edit page
  $('#additional_cost_type').change(function () {
      let type = $(this).val();
      ...
      // Check if freight row already exists
      if($('.freight-row').length > 0) {
          // Update existing freight row
          let row = $('.freight-row');
          row.find('span[name="freight_name"]').text('Freight Charges (' + type + ')');
          row.find('input[name="freight_type"]').val(type);
          row.find('input[name="freight_cost"]').val(amount);
          row.find('span[name="freight_taxable_value"]').text(amount.toFixed(2));
          row.find('span[name="freight_sub_total"]').text(amount.toFixed(2));
          calculateGrandTotal();
      ...
      // Get current row count for Sr.No
      let srNo = $("#product_table_body tr:not(.freight-row)").length + 1;
      ...
      cols += '<td><textarea name="freight_remark" rows="1" cols="20">Transport charges</textarea></td>';
      cols += '<td>NA</td>';
      cols += '<td><input type="number" class="form-control" name="freight_quantity" value="1" min="1" style="width:100%;"></td>';
      cols += '<td>NA</td>';
      cols += '<td>0.00</td>';
      cols += '<td><input type="number" class="form-control" name="freight_cost" value="' + amount.toFixed(2) + '" step="0.01" min="0" style="width:100%;"></td>';
      cols += '<td>—</td>';
      cols += '<td class="text-right"><span name="freight_taxable_value">' + amount.toFixed(2) + '</span></td>';
      cols += '<td class="tax_td">N/A</td>';
      cols += '<td class="text-right"><span name="freight_sub_total">' + amount.toFixed(2) + '</span></td>';
