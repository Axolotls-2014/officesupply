...
<form role="form" method="POST" name="importTaxForm" id="importTaxForm" enctype="multipart/form-data" action="<?=base_url('tax/import_tax')?>">
  <div class="modal-header text-left">
    <h4 class="modal-title"><?php echo $this->lang->line('tax_import_tax');?></h4>
    <button type="submit" name="submit" id="importTaxubmit" class="btn btn-primary" style="float: right"><?=$this->lang->line('submit')?></button>
  </div>
  <div class="modal-body">

  <div class="form-group row">
        <label for="csvfile" class="col-sm-4 col-form-label">
          <?=$this->lang->line("tax_csv_file")?>
        </label>
        <div class="col-sm-8">
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-8">
          <input type="checkbox" name="update_tax" value="1"> Update only tax<br/>
          <input type="checkbox" name="create_tax" value="1"> Create new tax though it exist <br/>
        </div>
      </div> 
      ...
      <div class="row">
        <div class="col-md-12" style="opacity: 0.7; border-radius: 5px; background-color: #F8E4A4; font-size: 16px;">
          <div class="m-4">
            Download sample file  <a href="<?=base_url('assets/sample_files/sample_tax.csv')?>" target="_blank" class="btn btn-info" style="float:right">Click here to Download</a>  
          </div>
        </div>
      </div>
...
<script>
  $(document).ready(function() {
    $('#csvfile').change(function(event) {
      ...
      $.ajax({
          type: 'POST',
          url: '<?php echo base_url('tax/get_import_tax')?>',
          data: formData,
          processData: false,
          contentType: false,
          success: function (response) {

            let resultHTML = '<h6 style="margin-top:10px;">Existing Tax:</h6> ';
            ...
            else 
            {
              ...
              resultHTML += '<tr>';
                resultHTML += '<td colspan="2"> No matched tax found. </th>';
