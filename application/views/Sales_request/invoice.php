...
<main class="main-wrapper">
    ...
    <div style="font-family: Arial, sans-serif; line-height: 1.6;">
        ...
        <!-- HEADER: Title centered + logo right -->
    ...
    <!-- Logo on right -->
    <img src="<?= config_item('base_url2') . '/assets/images/0/' . $company_setting->logo; ?>" 
         style="width: 150px; position: absolute; right: 0; top: -5px;">

    <!-- Spacer (fix height so text below does not overlap) -->
    ...
    <hr>
        ...
        <h4 style="text-align:center; margin:0;"><?= $company_setting->company_name; ?></h4>
            ...
            <table>
                ...
                <!-- Billing/Shipping/Transport -->
                 <tr class="heading">
                    ...
                    <td colspan="2" style="width:20%;">Purchase Requests Details</td>
                </tr>
                ...
                <tr class="details">
                    <td colspan="4" style="width:35%; vertical-align: top;">
                        ...
                        <?php else: ?>
                            ...
                            Email: <?= $added_by_user->email ?? 'N/A'; ?><br>
                    ...
                    <td colspan="2" style="width:20%; vertical-align: top;">
                        <strong>Purchase Order No:</strong> <?= $sale->reference_no; ?><br>
                        <strong>Purchase Order Date:</strong> <?= date('d-m-Y', strtotime($sale->invoice_date)); ?><br>
                    </td>
                </tr>
    ...
    //             if($order->product_name == 'A4 PAPER BINDAL 75 GSM') { 
    //     echo '<pre>'; 
    //     print_r($order); 
    //     echo '</pre>'; 
    // }
                   ...
                   $cgst_amount = ($order->taxable_value * $order->cgst_tax) / 100;
$sgst_amount = ($order->taxable_value * $order->sgst_tax) / 100;
$igst_amount = ($order->taxable_value * $order->igst_tax) / 100;
...
<!--                    <td>-->
<!--<?= $order->cost > 0 ? number_format($order->cost, 2) : number_format($order->subtotal / $order->quantity, 2); ?>-->
<!--</td>-->
...
if ($order->cgst_tax > 0) {
    echo '₹ ' . number_format($cgst_amount, 2) . ' (' . $order->cgst_tax . '%)<br>';
    $hasTax = true;
}

if ($order->sgst_tax > 0) {
    echo '₹ ' . number_format($sgst_amount, 2) . ' (' . $order->sgst_tax . '%)<br>';
    $hasTax = true;
}

if ($order->igst_tax > 0) {
    echo '₹ ' . number_format($igst_amount, 2) . ' (' . $order->igst_tax . '%)<br>';
    $hasTax = true;
}
