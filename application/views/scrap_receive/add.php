  ...
  <div class="wrapper">
    <div class="content-wrapper">
      ...
      <section class="content">
        <div class="row">
          <div class="col-md-12">
            <form class="form-horizontal" name="addScrapreceiveForm" id="addScrapreceiveForm" method="POST" action="<?=base_url('pack_slip/add')?>">
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title"><?=$this->lang->line('pack_slip_add')?></h3>
                  <div class="card-tools">
                    <div class="btn-group">
                      <button type="submit" name="submit" id="ScrapreceiveSubmit" class="btn btn-info"><?=$this->lang->line('pack_slip_save')?></button>
                    </div>
                  </div>
                </div>
                <div class="card-body">
                ...
                <div class="card-footer">
                  ...
                  <!-- <button type="submit" name="submit" id="ScrapreceiveSubmit" class="btn btn-info"><?=$this->lang->line('pack_slip_add')?></button> -->
...
<script type="text/javascript">
  ...
  $(document).ready(function(e){
      ...
      $("table.product_table").on('change keyup', 'input[name="cost"], input[name="quantity"], input[name="free_quantity"], select[name^="item_discount"]', function (event) {
      ...
      $(document).on('change','input[name="quantity"]',function(event){
        ...
        var freeQuantity = calculateFreeQuantity(product_id,ordered_qty);
        // alert('freeQuantity:'+ freeQuantity);
        tr.find('input[name="quantity"]').val(ordered_qty);
        tr.find('input[name="free_quantity"]').val(freeQuantity);
        tr.find('span[name="total_quantity"]').text(ordered_qty + freeQuantity);
      ...
      function calculateFreeQuantity(productId, orderedQuantity) {
        // let freeQuantity = 0;
        ...
        const promotions = JSON.parse(promotionsString);
        ...
        let globalRules_str = '';
        globalRules = promotions.find(promo => promo.product_id === '0');
        if (globalRules) {
          globalRules = globalRules.configuration.split('|').map(rule => JSON.parse(rule));
          // alert(globalRules);
          // alert((JSON.stringify(globalRules)).substr(1, (JSON.stringify(globalRules)).length - 2));
        ...
        productRules = promotions.find(promo => promo.product_id === productId);
        if (productRules) {
      ...
      function calculateRow(row)
      {
        ...
        else
        {
          ...
          row.find('span[name="total_quantity"]').text(parseFloat(quantity)+parseFloat(free_quantity));
          // set box and case
          row.find('span[name="no_of_case"]').text( Math.floor((quantity+free_quantity) / case_size));
          row.find('span[name="no_of_box"]').text( (quantity+free_quantity) % case_size);
      ...
      $('#addScrapreceiveForm').submit(function(e){
        // e.preventDefault();

        var isError = false;
        $('#packSlipSubmit').text('<?=$this->lang->line("please_wait")?>').attr('disabled','disabled');

        $('form#addScrapreceiveForm .field_validation').each(function() {
            var id    = $(this).attr('id');
            var value = $(this).val();
            var field = $(this).attr('placeholder');

            if(value==null || value==""){
              $("form#addScrapreceiveForm #err_"+id).text(field+ " field is required.").fadeIn('slow');
              $('form#addScrapreceiveForm #'+id).addClass('is-invalid');
              $('form#addScrapreceiveForm #'+id).removeClass('is-invalid');
              isError = true;
            }
            else
            {
              $("form#addScrapreceiveForm #err_"+id).text("").fadeOut('slow');
              $('form#addScrapreceiveForm #'+id).removeClass('is-invalid');
              $('form#addScrapreceiveForm #'+id).addClass('is-valid');
            }
        });
      ...
      $("form#addScrapreceiveForm .field_validation").on("blur keyup change",  function (event){
        var id    = $(this).attr('id');
        var value = $(this).val();
        var field = $(this).attr('placeholder');
        ...
        if(value==null || value==""){
          $("form#addScrapreceiveForm #err_"+id).text(field+ " field is required.").fadeIn('slow');
          $('form#addScrapreceiveForm #'+id).addClass('is-invalid');
          $('form#addScrapreceiveForm #'+id).removeClass('is-valid');
          return false;
        }
        else{
          $("form#addScrapreceiveForm #err_"+id).text("").fadeOut('slow');
