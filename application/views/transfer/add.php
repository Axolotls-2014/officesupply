  ...
  <div class="wrapper">
    <div class="content-wrapper">
          ...
          <div class="row mb-2">
            <div class="col-sm-12">
              <ol class="breadcrumb breadcrumb-custom float-sm-left">
                ...
                <li class="breadcrumb-item "><a href="<?=base_url('transfer')?>"><?=$this->lang->line('header_transfer')?></a></li>
                <li class="breadcrumb-item active"><?=$this->lang->line('transfer_add')?></li>
              </ol>
            </div>
          </div>
      </section>
      ...
      <section class="content">
        <div class="row">
          <div class="col-md-12">
            <form class="form-horizontal" name="addTransferForm" id="addTransferForm" method="POST" action="<?=base_url('transfer/add')?>">
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title"><?=$this->lang->line('transfer_add')?></h3>
                    <div class="card-tools">
                      <ul class="nav nav-pills ml-auto">
                        <li class="nav-item ml-2">
                          <a class="nav-link active" href="<?=base_url('transfer')?>" data-tt="tooltip" title="Click here to show transfer list"><i class="fas fa-arrow-left mr-2"></i>Back</a>
                        </li>
                      </ul>
                    </div>
                </div>
                <div class="card-body">
                  <div class="col-sm-12">
                    <div class="row">
                      ...
                      <div class="col-sm-4">
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control  datepicker" id="transfer_date" name="transfer_date" value="<?=date('d-m-Y');?>">
                      ...
                      <div class="col-sm-4">
                        <div class="form-group">
                          ...
                          <input type="hidden" name="to_warehouse_state_id" id="to_warehouse_state_id" value="">
                          <input type="hidden" name="to_warehouse_country_id" id="to_warehouse_country_id" value="">
                    ...
                    <div class="row">
                      <div class="col-sm-12 ">
                        <div class="well">
                          <div class="row">
                            <!-- <div class="col-sm-12">
                              <a href="" data-toggle="modal" data-tt="tooltip" data-target="#add_service_modal" class="float-right"><?=$this->lang->line('transfer_add_new_service')?></a>
                            </div> -->
                            <div class="col-sm-12 search">
                              <span class="fa fa-search"></span>
                              <input id="search_product" class="form-control search_product"  type="text" name="search_product"  placeholder="<?=$this->lang->line('transfer_item_search')?>">
                    ...
                    <div class="row">
                      <div class="col-sm-12">
                        <div class="form-group">
                          <label><?=$this->lang->line('transfer_items')?></label>
                          <table class="table items table-striped table-bordered table-condensed table-hover product_table" name="product_data" id="product_data">
                            <thead>
                              <tr>
                                ...
                                <th class="span2" width="10%"><?=$this->lang->line('sale_batch_no')?></th>
                                <th class="span2" width="10%"><?=$this->lang->line('sale_cost')?></th>
                          ...
                          <table class="table table-striped table-bordered table-condensed table-hover total_data">
                            <tr style="display: none">
                              <td align="right" width="66%"> <?=$this->lang->line('transfer_total_discount')?>(<?=$currency?>)</td>
                            ...
                            <tr  style="display: none">
                              <td align="right" width="66%"><?=$this->lang->line('transfer_total_taxable_value')?>(<?=$currency?>)</td>
                            ...
                            <tr  style="display: none">
                              <td align="right" width="66%"><?=$this->lang->line('transfer_total_tax')?>(<?=$currency?>)</td>
                              <td align='right' class="text-success" width="34%">
                            ...
                            <tr>
                              <td align="right" width="66%"><?=$this->lang->line('transfer_total')?>(<?=$currency?>)</td>
                ...
                <div class="card-footer">
                  ...
                  <button type="submit" name="submit" id="saleSubmit" class="btn btn-info"><?=$this->lang->line('transfer_add')?></button>
                  <!-- <button type="submit" name="submit" id="saleSubmitPayNow" value="pay" name="pay" class="btn btn-info">Add Sale & Pay Now</button>                      -->
                  <span class="btn btn-default float-right" id="cancel" onclick="cancel('sale')"><?=$this->lang->line('transfer_cancel')?></span>
                </div>
            </form>
          </div>
        </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header warning-header">
        <h5 class="modal-title" id="exampleModalLabel"><?=$this->lang->line('transfer_message_label')?></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <?=$this->lang->line('empty_transfer_warning_label')?>
      </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function(e){
      ...
      $('#from_warehouse_id').change(function(e){
        ...
        if(warehouse_id != '')
        {
          $("#from_warehouse_id").closest('.row').siblings().css('display','block');
      ...
      $('#to_warehouse_id').change(function(e){
        ...
        if(to_warehouse_id != '')
        {
          ...
          {
            $("#to_warehouse_id").closest('.row').siblings().fadeIn(10);  
          }
          ...
          $.ajax({
              ...
              success: function(data){
                ...
                $('#to_warehouse_state_id').val(warehouse.state_id);
                $('#to_warehouse_country_id').val(warehouse.country_id);
        ...
        {
          $("#to_warehouse_id").closest('.row').siblings().css('display','none');
        }
      }); 
      ...
      {
        var to_warehouse_country_id = $('#to_warehouse_country_id').val();
        var to_warehouse_state_id   = $('#to_warehouse_state_id').val();
        ...
        if(company_country_id == to_warehouse_country_id)
        {
          
            if(company_state_id == to_warehouse_state_id)
            {
        ...
        var newRow = $('<tr class="service_row">');
            ...
            cols += '<td>' 
                      +'<span id="cost_span">'
                        +'<input type="number" class="form-control text-right" name="cost" step="0.01" value="'+product.cost+'" readonly>'
                      +'</span>'
                    +'</td>';
            cols += '<td>' 
                      +'<span id="selling_price_span">'
                        +'<input type="number" class="form-control text-right" name="selling_price" step="0.01" value="'+product.selling_price+'" readonly>'
                      +'</span>'
                    +'</td>';
            cols += '<td>' 
                    +'<span id="price_span">'
      ...
      $('#addTransferForm').submit(function(e){
        ...
        if(isError == true)
        {
          $('#saleSubmit').text('<?=$this->lang->line("transfer_add")?>').removeAttr('disabled');
          return false;
        }
        else 
        {
