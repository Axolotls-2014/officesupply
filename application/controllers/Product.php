...
//     $target_date = ($date_input) ? date('Y-m-d', strtotime($date_input)) : null;
//     $pid = $this->input->post('product_id');
//     $qty_filter = $this->input->post('quantity');

//     $list = $this->product_model->get_all_movements_ledger($target_date, $pid);
...
//         $item->closing_stock = $running_stock[$p_id] + $item->in_qty - $item->out_qty;
//         $unit_rate = ($item->pur_price > 0) ? $item->pur_price : $item->p_master_cost;
//         $item->closing_value = $item->closing_stock * $unit_rate;
//         $running_stock[$p_id] = $item->closing_stock;
       
//         // Apply filters
//         $show = true;
//         if($qty_filter == QUANTITY_GREATER_THEN_ZERO && $item->closing_stock <= 0) $show = false;
//         if($qty_filter == QUANTITY_ZERO && $item->closing_stock != 0) $show = false;
//         if($qty_filter == QUANTITY_NEGATIVE && $item->closing_stock >= 0) $show = false;
        
//         if($this->input->post('product_status') && $item->p_status != $this->input->post('product_status')) $show = false;

//         if($show) $processed_data[] = $item;
//     }

//     $processed_data = array_reverse($processed_data); // Newest First
//     $data = array();
...
//         $row[] = '<button class="btn btn-info btn-xs"><i class="fas fa-eye"></i></button>';
//         $data[] = $row;
//     }
...
public function ajax_list_stock() {
    ...
    $target_from = ($from_input) ? date('Y-m-d', strtotime($from_input)) : null;
    $target_to   = ($to_input) ? date('Y-m-d', strtotime($to_input)) : date('Y-m-d'); 
    
    $pid = $this->input->post('product_id');
    $qty_filter = $this->input->post('quantity');

    // IMPORTANT: Get history up to the "TO_DATE" to calculate correct stock
    $list = $this->product_model->get_all_movements_ledger($target_to, $pid);
    ...
    foreach ($list as $item) {
        ...
        // --- 1. ALWAYS CALCULATE (Math for every row) ---
        $item->closing_stock = $running_stock[$p_id] + $item->in_qty - $item->out_qty;
        $unit_rate = ($item->pur_price > 0) ? $item->pur_price : $item->p_master_cost;
        $item->closing_value = $item->closing_stock * $unit_rate;
        $running_stock[$p_id] = $item->closing_stock;

        // --- 2. FILTER DISPLAY (Only show what is in range) ---
        $item_date = date('Y-m-d', strtotime($item->date));
        $show = true;

        // Date Range logic
        if ($target_from && $target_to) {
            if ($item_date < $target_from || $item_date > $target_to) {
                $show = false; // Hide if outside From-To range
            }
        }
    ...
    $processed_data = array_reverse($processed_data); // Show newest movements at the top
    $data = array();
