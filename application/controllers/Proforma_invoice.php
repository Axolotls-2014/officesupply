...
class Proforma_invoice extends MY_Controller {
	...
	{
		if(!$this->permission_model->has_permission('list_proforma_invoice') && !$this->permission_model->has_permission('list_all_proforma_invoice'))
		{
		...
		else
		{
			...
			$data['proforma_invoices'] 				= $this->proforma_invoice_model->get_proforma_invoice_records();
            $data['total_proforma_invoice']		= $this->proforma_invoice_model->get_total_proforma_invoice();
			
// 			$log_data = array(
//                   "user_id"     => $this->session->userdata("user_id"),
//                   "module"      => "proforma_invoice",
//                   "user_action" => 0,
//                   "description"	=> "User has viewed list of proforma invoice."
//                 );

// 			$this->log_data_model->add_record($log_data);

			$this->load->view('proforma_invoice/list',$data);
		}
	}
    ...
    else
    {
        if($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            if($this->form_validation->run()==FALSE)
            {
                ...
                $this->load->view('proforma_invoice/add',$data);
            }
            else
            {
                // Process profit data
                ...
                $reference_no = $this->proforma_invoice_model->get_lastest_sequence_number();
                ...
                foreach ($optional_fields as $field) {
                    if (!empty($this->input->post($field))) {
                        $proforma_invoice_data[$field] = $this->input->post($field);
                    }
                }

                $lori_date = $this->input->post('lori_date');
                if($lori_date != '') {
                    $proforma_invoice_data['lori_date'] = date('Y-m-d',strtotime($lori_date));
                }

                $ewaybill_transporter_doc_date = $this->input->post('ewaybill_transporter_doc_date');
                if($ewaybill_transporter_doc_date != '') {
                    $proforma_invoice_data['ewaybill_transporter_doc_date'] = date('Y-m-d',strtotime($ewaybill_transporter_doc_date));
                }

                // Begin transaction
                $this->db->trans_begin();

                if($id = $this->proforma_invoice_model->add_proforma_invoice_record($proforma_invoice_data))
                {
                    $entered_proforma_invoice = $this->proforma_invoice_model->get_proforma_invoice_single_record($id);
                    $customer = $this->customer_model->get_single_record($entered_proforma_invoice->customer_id);
                    ...
                    $log_data = array(
                        ...
                        "entry_id" => $id,
                        "user_action" => 1,
                        "data" => json_encode((array)$entered_proforma_invoice),
                        "description" => 'Proforma invoice (Reference No. -' .$reference_no .') is added successfully.'
                    );
                    $this->log_data_model->add_record($log_data);

                    $proforma_invoice_items = $this->input->post('proforma_invoice_items');
                    $proforma_invoice_items_array = explode("|", $this->input->post('proforma_invoice_items'));

                    for ($i=0; $i < sizeof($proforma_invoice_items_array); $i++) { 
                        $temp_proforma_invoice_item = (array)json_decode($proforma_invoice_items_array[$i]);
                        $temp_proforma_invoice_item['proforma_invoice_id'] = $id;
                        
                        // Handle date fields
                        if(isset($temp_proforma_invoice_item['expiry_date']) && 
                          ($temp_proforma_invoice_item['expiry_date'] == '' || $temp_proforma_invoice_item['expiry_date'] == '0000-00-00')) {
                            unset($temp_proforma_invoice_item['expiry_date']);
                        } elseif(isset($temp_proforma_invoice_item['expiry_date'])) {
                            $temp_proforma_invoice_item['expiry_date'] = date('Y-m-d',strtotime($temp_proforma_invoice_item['expiry_date']));
                        }

                        if(isset($temp_proforma_invoice_item['mfg_date']) && 
                          ($temp_proforma_invoice_item['mfg_date'] == '' || $temp_proforma_invoice_item['mfg_date'] == '0000-00-00')) {
                            unset($temp_proforma_invoice_item['mfg_date']);
                        } elseif(isset($temp_proforma_invoice_item['mfg_date'])) {
                            $temp_proforma_invoice_item['mfg_date'] = date('Y-m-d',strtotime($temp_proforma_invoice_item['mfg_date']));
                        }
                        ...
                        $this->proforma_invoice_model->add_proforma_invoice_item_record($temp_proforma_invoice_item);
                    }

                    // Commit transaction
                    $this->db->trans_commit();

                    $this->session->set_flashdata('success', 'Proforma invoice (Reference No. -' .$reference_no .') is added successfully.');
                ...
                else
                {
                    ...
                    $this->session->set_flashdata('failure', 'Proforma invoice (Reference No. -' .$reference_no .') is failed to add.');
                    redirect('proforma_invoice','refresh');
                }
            }
        }
        else
        {
            ...
            if(PROFORMA_INVOICE_RESTRICTION == 'YES' && $customer != null)
            {
...
public function edit($id = null)
{
    if(!$this->permission_model->has_permission('edit_proforma_invoice')) {
        ...
        if($this->input->server('REQUEST_METHOD') === 'POST') {
            $proforma_invoice_id = $this->input->post('id');
            $old_proforma_invoice = $this->proforma_invoice_model->get_proforma_invoice_single_record($proforma_invoice_id);
            ...
            if($this->form_validation->run() == FALSE) {
                ...
                // Optional e-way bill fields
                $ewaybill_fields = [
                    'ewaybill_dispatch_from', 'ewaybill_mode_of_transportation', 'ewaybill_subtype',
                    'ewaybill_doctype', 'ewaybill_transporter_name', 'ewaybill_transporter_gstin',
                    'ewaybill_distance_of_transportation', 'ewaybill_transporter_doc_no',
                    'ewaybill_vehicle_no', 'ewaybill_vehicle_type'
                ];
                
                foreach ($ewaybill_fields as $field) {
                    if (!empty($$field)) {
                        $proforma_invoice_data[$field] = $$field;
                    }
                }
                ...
                foreach ($date_fields as $field => $value) {
                    if (!empty($value)) {
                        $proforma_invoice_data[$field] = date('Y-m-d', strtotime($value));
                    }
                }

                if($this->proforma_invoice_model->edit_proforma_invoice_record($proforma_invoice_data, $proforma_invoice_id)) {
                    $entered_proforma_invoice = $this->proforma_invoice_model->get_proforma_invoice_single_record($proforma_invoice_id);

                    $log_data = array(
                        "user_id" => $this->session->userdata("user_id"),
                        "module" => "proforma_invoice",
                        "user_action" => 2,
                        "b_data" => json_encode((array)$old_proforma_invoice),
                        "data" => json_encode((array)$entered_proforma_invoice),
                        "description" => 'Proforma invoice (Reference No. -' . $reference_no . ') is updated successfully.'
                    );
                    $this->log_data_model->add_record($log_data);
                    $this->proforma_invoice_model->remove_proforma_invoice_item_records($proforma_invoice_id);

                    $proforma_invoice_items_array = explode("|", $this->input->post('proforma_invoice_items'));

                    foreach ($proforma_invoice_items_array as $item) { 
                        $temp_proforma_invoice_item = (array)json_decode($item);
                        $temp_proforma_invoice_item['proforma_invoice_id'] = $proforma_invoice_id;

                        // Handle supplier_id if exists
                        if(isset($temp_proforma_invoice_item['supplier_id'])) {
                            $temp_proforma_invoice_item['supplier_id'] = $temp_proforma_invoice_item['supplier_id'];
                        }

                        // Handle purchase_cost if exists
                        if(isset($temp_proforma_invoice_item['purchase_cost'])) {
                            $temp_proforma_invoice_item['purchase_cost'] = $temp_proforma_invoice_item['purchase_cost'];
                        }

                        // Process date fields
                        $date_fields = ['expiry_date', 'mfg_date'];
                        foreach ($date_fields as $field) {
                            if (isset($temp_proforma_invoice_item[$field]) && 
                                $temp_proforma_invoice_item[$field] != '' && 
                                $temp_proforma_invoice_item[$field] != '0000-00-00') {
                                $temp_proforma_invoice_item[$field] = date('Y-m-d', strtotime($temp_proforma_invoice_item[$field]));
                            } else {
                                unset($temp_proforma_invoice_item[$field]);
                            }
                        }
                    ...
                    $this->session->set_flashdata('success', 'Proforma invoice (Reference No. -' . $reference_no . ') is updated successfully.');
                    redirect('proforma_invoice', 'refresh');
                } else {
                    $this->session->set_flashdata('failure', 'Proforma invoice (Reference No. -' . $reference_no . ') is failed to update.');
                    redirect('proforma_invoice', 'refresh');
                }
            }
        } else {
            ...
            if($id != null) {
                $data['proforma_invoice'] = $this->proforma_invoice_model->get_proforma_invoice_single_record($id);

                if($data['proforma_invoice'] != null) {
                    $data['countries'] = $this->utility_model->get_countries();

                    if($data['proforma_invoice']->customer_shipping_country_id != '') {
                        $data['states'] = $this->utility_model->get_states($data['proforma_invoice']->customer_shipping_country_id);
                    }
    
                    if($data['proforma_invoice']->customer_shipping_state_id != '') {
                        $data['cities'] = $this->utility_model->get_cities($data['proforma_invoice']->customer_shipping_state_id);
                    }                       

                    $data['proforma_invoice_items'] = $this->proforma_invoice_model->get_proforma_invoice_item_records($id);
                    
                    $this->load->model('supplier_model');
                                foreach($data['proforma_invoice_items'] as &$item) {
                                    $item->product_vendors = $this->supplier_model->get_vendors_by_product($item->product_id);
                                }
        
                    
                    $data['customers'] = $this->customer_model->get_records();
                    $data['customer_detail'] = $this->customer_model->get_single_record($data['proforma_invoice']->customer_id);
                    $data['discounts'] = $this->discount_model->get_records();
                    $data['company_setting'] = $this->company_settings_model->get_company_records();
                    $data['customer_shipping_addresses'] = $this->customer_model->get_customer_shipping_addresses($data['proforma_invoice']->customer_id);
                    $data['warehouses'] = $this->warehouse_model->get_records();
                    $data['suppliers'] = $this->supplier_model->get_records();
                    $data['promotions'] = $this->promotion_model->get_records();

                    $this->load->view('proforma_invoice/edit', $data);
                } else {
	...
	function delete()
	{
		if(!$this->permission_model->has_permission('delete_proforma_invoice'))
		{
			$this->load->view('errors/html/error_restricted'); 
		}
		else
		{
			...
			$proforma_invoice 				= $this->proforma_invoice_model->get_proforma_invoice_single_record($id);
			$data = array('delete_status' => 1);
			if($this->proforma_invoice_model->edit_proforma_invoice_record($data,$id))
			{
				$customer 		= $this->customer_model->get_single_record($proforma_invoice->customer_id);

				$this->session->set_flashdata('success', 'Proforma invoice (Reference No. -'.$proforma_invoice->reference_no.') is deleted successfully.');
				redirect('proforma_invoice','refresh');
			}
			else
			{
				$this->session->set_flashdata('failure', 'Proforma invoice (Reference No. -'.$proforma_invoice->reference_no.') is failed to delete.');
				redirect('proforma_invoice','refresh');
			}	
	...
	public function view($id = null)
	{
		...
		else
		{
			...
			if (!$this->input->is_ajax_request()) 
			{
				if($id != null)
				{
					...
					if($data['proforma_invoice'] != null)
					{
						...
						$data['discounts']			= $this->discount_model->get_records();
						$data['company_setting']= $this->company_settings_model->get_company_records();	
                        $warehouse_id = $data['proforma_invoice']->warehouse_id;
                        $data['warehouse'] = $this->warehouse_model->get_single_record($warehouse_id);
						$this->load->view('proforma_invoice/view',$data);
					}
					else
					{
						$this->session->set_flashdata('failure',  'You have tried to access broken URL.');
			...
			else
			{
				...
				$proforma_invoice_view_data['proforma_invoice'] = $this->proforma_invoice_model->get_proforma_invoice_single_record($id);
				$proforma_invoice_view_data['customer_detail'] 	= $this->customer_model->get_single_record($proforma_invoice_view_data['proforma_invoice']->customer_id);	
			
				$response['proforma_invoice_view']	 						= $this->load->view('proforma_invoice/ajax/view',$proforma_invoice_view_data,TRUE);
  ...
  public function pdf($id = null)
	{
		...
		{	
			...
			$data['company_setting']	= $this->company_settings_model->get_company_records();	
             $warehouse_id = $data['proforma_invoice']->warehouse_id;
                        $data['warehouse'] = $this->warehouse_model->get_single_record($warehouse_id);
			$html = $this->load->view('proforma_invoice/pdf',$data,true);
			...
			$this->pdf->render();
			$this->pdf->stream("ProformaInvoice-".$data['proforma_invoice']->reference_no.".pdf", array("Attachment"=>1));
		}
		else
		{
			$this->session->set_flashdata('failure',  'You have tried to access broken URL.');
  ...
  function convert_from_quotation($quotation_id)
	{	
		...
		//$quotation_id 		= $this->input->post('quotation_id');
		 ...
		 $proforma_invoice = $this->proforma_invoice_model->get_proforma_invoice_single_record_by_quotation_id($quotation_id);
      ...
      // exit;
			...
			$proforma_invoice_data = array(
                                                  ...
                                                  // "customer_shipping_pincode"	=> $customer->shipping_pincode,
													...
													"user_id" 						=> $this->session->userdata('user_id'),
													 "additional_cost_type"  =>$quotation->additional_cost_type,
                                                    "additional_cost_amount"  =>$quotation->additional_cost_amount,
                                                    "total_purchase_cost" => $quotation->total_purchase_cost,
                                                    "total_selling_price" => $quotation->total_selling_price,
                                                    "gross_profit_loss" => $quotation->gross_profit_loss,
                                                    "profit_margin" => $quotation->profit_margin,
                                                    
                                                            'freight_selling_price' => $quotation->freight_selling_price,
                                                            'freight_taxable_value' => $quotation->freight_taxable_value,
                                                            'freight_sub_total' => $quotation->freight_sub_total,
                                                            'freight_tax_rate' => $quotation->freight_tax_rate,
                                                            'freight_tax_amount' => $quotation->freight_tax_amount   
												);
			...
			if($id = $this->proforma_invoice_model->add_proforma_invoice_record($proforma_invoice_data))
			{
				...
				$log_data = array(
									...
									"user_action"	=> 1,
									"data"			=> json_encode((array)$entered_proforma_invoice),
									"description"   => 'Proforma invoice (Reference No. -'  .$proforma_invoice_data['reference_no'] .') is added successfully.'
								);
				...
				foreach ($quotation_items as $value) {
					...
					$temp_proforma_invoice_item = array(
													...
													// echo '<pre>';
													// print_r($temp_proforma_invoice_item);
													// exit;
				...
				$this->session->set_flashdata('success',  'Proforma invoice (Reference No. -'  .$proforma_invoice_data['reference_no'] .') is added successfully.');
				redirect('proforma_invoice/view/'.base64_encode($id),'refresh');
			}
			else
			{
				// rollback transaction
				...
				$this->session->set_flashdata('failure',  'Proforma invoice (Reference No. -'  .$proforma_invoice_data['reference_no'] .') is failed to add.');
				redirect('proforma_invoice/view/'.base64_encode($id),'refresh');
			}	
		}
		else
		{
			$this->session->set_flashdata('success',  'Proforma invoice (Reference No. -'  .$proforma_invoice->reference_no.') is already exist');
			redirect('proforma_invoice/view/'.base64_encode($proforma_invoice->id),'refresh');
		}
	}

  public function get_proforma_invoice_items()
	{
    ...
    foreach ($proforma_invoice_ids as $proforma_invoice_id) {
     $this->proforma_invoice_model->edit_proforma_invoice_record($data,$proforma_invoice_id);
    }
    ...
    $data['proforma_invoice_items'] = $this->proforma_invoice_model->get_proforma_invoice_item_records($proforma_invoice_ids,true);

    
    

		// $response['product_table_body'] = $this->load->view('proforma_invoice/ajax/pack_slip_items',$data,TRUE);
	...
	public function import_product()
  {
      ...
      else 
      {
        ...
        else 
        {
          if ($this->input->is_ajax_request()) 
          {
            ...
            $response = array();
            $response['code'] = 1;
            $response['import_product_modal_body'] = $this->load->view('proforma_invoice/ajax/import_product_modal_body', $data, TRUE);
	...
	public function send_email($proforma_invoice_id = null)
	{
		...
		if($this->input->server('REQUEST_METHOD') === 'POST')
		{
			...
			{	
				...
				if($id = $this->email_model->add_email_sent_record($email_data))
				{
					...
					$proforma_invoice_items 								= $this->proforma_invoice_model->get_proforma_invoice_item_records($proforma_invoice->id);
					$customer_detail						= $this->customer_model->get_single_record($proforma_invoice->customer_id);
					...
					$data['proforma_invoice_items'] 				= $proforma_invoice_items;
					...
					$pdf_filename = "Proforma invoice-".$data['proforma_invoice']->reference_no.".pdf"; // Set your desired filename

          $cid = $company_setting->cid;
          $pdf_path = FCPATH . 'assets/documents/' . $cid . '/proforma_invoice/' . $pdf_filename;

          // Create the target folder if it doesn't exist
          $targetDir = FCPATH . 'assets/documents/' . $cid . '/proforma_invoice/';
          if (!file_exists($targetDir)) {
              mkdir($targetDir, 0777, true); // The third parameter creates nested directories if needed
					...
					else
					{
						...
						// $mail_data['Body'] 							= $this->load->view('proforma_invoice/pdf',$data,true);
					// commit transaction
					$this->db->trans_commit();
		...
		else
		{
			...
			$data['proforma_invoice'] 	= $this->proforma_invoice_model->get_proforma_invoice_single_record($proforma_invoice_id);
			$data['customer']		= $this->customer_model->get_single_record($data['proforma_invoice']->customer_id);
			$data['company_setting']	= $this->company_settings_model->get_company_records();	

			$data['email_template'] 	= $this->email_template_model->get_record_by_module(EMAIL_TEMPLATE_MODULE_PROFORMA_INVOICE);
	  	...
	  	$response['code']  								= 1;
			...
			$response['email_modal_body'] 	= $this->load->view('proforma_invoice/ajax/email_modal_body',$data,TRUE);
	...
	public function ajax_list()
  {
    ...
    foreach ($list as $item){  
	      ...
	      $table_body   = '';
				...
				if($this->permission_model->has_permission('email_proforma_invoice'))
				{

					$table_body .= '<a href="#"  data-tt="tooltip" title="'.$this->lang->line('send_email').'" class="btn btn-success btn-sm text-white email-modal" data-proforma_invoice_id="'.$item->id.'">
													<i class="fas fa-at"></i>
												</a>';
				}
				...
				//View Button
			  if($this->permission_model->has_permission('view_proforma_invoice') || $this->permission_model->has_permission('view_all_proforma_invoice'))
				{	
					$table_body .= ' 
                            <a href="'.base_url('proforma_invoice/view/'.base64_encode($item->id)).'" class="btn btn-default btn-xs" data-tt="tooltip" title="View Proforma Invoice">
				...
				// Edit Button        
       	if($this->permission_model->has_permission('edit_proforma_invoice') && $item->sale_id === NULL)
				{	
            $table_body .= ' 
          										<a href="'.base_url('proforma_invoice/edit/'.base64_encode($item->id)).'" class="btn btn-info btn-xs"  data-tt="tooltip" title="Edit Proforma Invoice">
				...
				// Delete Button
				if($this->permission_model->has_permission('delete_proforma_invoice') && $item->sale_id === NULL)
				{	
          
            $table_body .= '
		                        <a href="#"  data-toggle="modal" data-target="#delete_proforma_invoice" data-tt="tooltip" title="Delete Proforma Invoice" class="btn btn-danger btn-xs delete_proforma_invoice" data-proforma_invoice_id="'.$item->id.'">
				...
				//Reference No
        $reference_no_html = '<a href="'.base_url('proforma_invoice/view/'.base64_encode($item->id)).'" data-tt="tooltip" title="'.$this->lang->line('proforma_invoice_view').'">'.$item->reference_no.'</a>';

         //Customer Name
        $customer_name_html = '<a href="'.base_url('customer/view/'.base64_encode($item->customer_id)).'" data-tt="tooltip" title="'.$this->lang->line('proforma_invoice_view_customer_detail').'">'.$item->customer_name.'</a>';
                      
				/* End Action column buttons*/

        $sale = $this->sale_model->get_sale_single_record_by_proforma_invoice_id($item->id);

        $checkbox_attributes = $sale ? 'disabled readonly' : '';
        
        $select_html = '<input type="checkbox" class="single_proforma_invoice" data-proforma_invoice_id="'.$item->id.'" '.$checkbox_attributes.'>';
  ...
  public function proforma_invoice_delete_confirmation()
  {
  	$proforma_invoice_id 				= $this->input->post('proforma_invoice_id');
  	$data['proforma_invoice'] 	= $this->proforma_invoice_model->get_proforma_invoice_single_record($proforma_invoice_id);

  	$response 					= array();
  	$response['proforma_invoice_delete_modal_body'] 	= $this->load->view('proforma_invoice/ajax/proforma_invoice_delete_modal_view',$data,TRUE);
    ...
    if (isset($_FILES["upload_document"])) 
		{
			...
			$targetDir = "./assets/documents/$cid/proforma_invoice/";
